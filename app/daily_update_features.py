...
for _, cal_row in upcoming.iterrows():
    try:
        c.execute("""
            INSERT OR IGNORE INTO ml_dataset (ticker, earnings_date)
            VALUES (?, ?)
        """, (cal_row['ticker'], cal_row['earnings_date'].strftime('%Y-%m-%d')))
    except:
        pass
conn.commit()
...
if df.empty:
    print("No new rows to populate. Everything up to date.")
    conn.close()
    exit()

print(f"Populating features for {len(df)} upcoming events...")

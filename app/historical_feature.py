...
# =============================================================================
# 1. Ensure all feature columns exist (same block you already have)
# =============================================================================
columns = [
...
# =============================================================================
# 2. Get the list of tickers that have an upcoming earnings date in the next 7 days
# =============================================================================
today = date.today()
cutoff = today + timedelta(days=7)

upcoming_tickers = pd.read_sql("""
    SELECT DISTINCT ticker
    FROM earnings_calendar
    WHERE earnings_date BETWEEN date('now') AND ?
""", conn, params=(cutoff.strftime("%Y-%m-%d"),))['ticker'].tolist()

if not upcoming_tickers:
    print("No upcoming tickers in the next 7 days. Exiting.")
    conn.close()
    exit()

print(f"Processing historical features for {len(upcoming_tickers)} upcoming tickers...")

# =============================================================================
# 3. For each of those tickers, select all rows in ml_dataset that still need features
#    (e.g. quality_curr IS NULL) and process them.
# =============================================================================
...
for ticker in upcoming_tickers:
    # Load all rows for this ticker that are missing features
    df_ticker = pd.read_sql("""
        SELECT ticker, earnings_date
        FROM ml_dataset
        WHERE ticker = ?
          AND quality_curr IS NULL
        ORDER BY earnings_date
    """, conn, params=(ticker,), parse_dates=['earnings_date'])

    if df_ticker.empty:
        continue

    print(f"  {ticker}: processing {len(df_ticker)} historical rows...")

    # Load price & earnings history once for this ticker
    ...
    for _, row in df_ticker.iterrows():
        ...
        # Compute all features (with point‑in‑time safety)
        ...
        conn.commit()
        time.sleep(0.15)   # gentle rate‑limit pause

print("Incremental historical feature population complete.")
conn.close()
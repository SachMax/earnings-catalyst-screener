...
try:
    c.execute("ALTER TABLE features ADD COLUMN hist_runup_avg REAL;")
...
for index,row in df_ed.iterrows(): 
    try:
        ...
        print(f"{ticker}: past run-up mean = {past_runup_mean}%")
    except Exception as e:
        print(f"{ticker}: error -> {e}")
        continue
    c.execute("UPDATE features SET hist_runup_avg = ? WHERE ticker = ? AND earnings_date = ?",
          (past_runup_mean, ticker, ed_clean.strftime("%Y-%m-%d")))
    conn.commit()
conn.close()